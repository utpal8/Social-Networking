			...
			database.collection("users").findOne({
				...
				if (user == null) {
					...
					if (request.files.coverPhoto.size > 0 && request.files.coverPhoto.type.includes("image")) {
						...
						coverPhoto = "public/images" + new Date().getTime() + "_" + request.files.coverPhoto.name;
						fileSystem.rename(request.files.coverPhoto.path, coverPhoto, function (error) {
							//
						});
		...
		app.post("/toggleJoinGroup", function (request, result) {
			...
			database.collection("users").findOne({
				...
				if (user == null) {
					result.json({
						"status": "error",
						"message": "User has been looged out. Please login again."
					});
				} else {
					database.collection("groups").findOne({
						"_id": ObjectId(_id)
						...
						if (group == null) {
							...
							for (var a = 0; a < group.members.length; a++) {
								...
								if (momber._id.toString() == user._id.toString()) {
									isMember = true;
									break;
								}
							}
								...
								database.collection("Users").updateOne({
									...
									database.collection("users").updateOne({
										$and: [{
											...
											result.json({
												"status": "success",
												"message": "Gorup join request has been accepted."
											});
									});
								});
							});
